...
const VimeoRenderer = ({ url, title }: { url: string; title?: string }) => {
  ...
  if (!vimeoId)
    return <GenericRenderer url={url} title={title || "Vimeo Video"} />;
  ...
  if (!url.includes("/embed")) {
    return (
      <a
        ...
        className="block w-full p-4 hover:bg-white/5 transition-colors group/map"
      >
        <div className="flex items-center gap-3 mb-2">
          <div className="p-2 bg-green-500/20 rounded-full text-green-400">
            <MapPin className="w-5 h-5" />
          </div>
          <span className="text-green-400/80 font-semibold text-sm">
            Google Maps
          </span>
        </div>
        <div className="pl-[3.25rem]">
          <p className=" text-lg  truncate">{title || "View Location"}</p>
          <p className="/40 text-xs truncate">{url}</p>
        </div>
      </a>
    );
  }
...
const InstagramRenderer = ({ url, title }: { url: string; title?: string }) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="block w-full p-4 hover:bg-white/5 transition-colors group/instagram bg-gradient-to-br from-[#833ab4]/10 via-[#fd1d1d]/10 to-[#fcb045]/10 border border-white/5 rounded-lg"
    >
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="">
          <p className=" text-lg font-medium truncate">
            {title || `View Instagram ${displayType}`}
          </p>
          <p className="/40 text-xs truncate">{url}</p>
        </div>
      </div>
...
const RedditRenderer = ({ url, title }: { url: string; title?: string }) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="block w-full p-4 hover:bg-white/5 transition-colors group/reddit bg-[#FF4500]/5 border border-[#FF4500]/20 rounded-lg"
    >
      <div className="flex items-center gap-3 mb-2">
      ...
      <div className="pl-[3.25rem]">
        <p className=" text-lg font-medium truncate">
          {inferredTitle || `View Reddit ${displayType}`}
        </p>
        <p className="/40 text-xs truncate">{url}</p>
      </div>
    </a>
...
const TwitterRenderer = ({ url, title }: { url: string; title?: string }) => {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="block w-full p-4 hover:bg-white/5 transition-colors group/twitter"
    >
      <div className="flex items-center gap-3 mb-2">
        <div className="p-2 bg-[#1DA1F2]/20 rounded-full text-[#1DA1F2]">
          <Twitter className="w-5 h-5" />
        </div>
        <span className="text-[#1DA1F2] font-semibold text-sm">
          Twitter / X
        </span>
      </div>
      <div className="pl-[3.25rem]">
...
const GithubRenderer = ({ url, title }: { url: string; title?: string }) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="block w-full p-4 hover:bg-white/5 transition-colors group/github"
    >
      <div className="flex items-center gap-3 mb-2">
        <div className="p-2 bg-white/10 rounded-full ">
          <Github className="w-5 h-5" />
        </div>
        <span className="/80 font-semibold text-sm">GitHub</span>
      </div>
      <div className="pl-[3.25rem] border-l-2 border-white/10 ml-4 py-1">
        <p className=" text-lg font-mono font-medium truncate">
          {title || repoPath}
        </p>
        <p className="/40 text-xs truncate mt-1">Repository</p>
      </div>
    </a>
  );
};
...
const ImageRenderer = ({ url, title }: { url: string; title?: string }) => {
  return (
    <div className="relative w-full flex justify-center bg-black/40 p-2">
      ...
      />
      {title && (
        <div className="absolute bottom-2 left-2 right-2 bg-black/60 p-2  text-xs rounded backdrop-blur-sm truncate">
          {title}
        </div>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <h4 className="text-base font-medium  truncate group-hover/link:underline">
          {title || url.split("/").pop()}
        </h4>
        <p className="text-xs /50 truncate">{url}</p>
      </div>
    </a>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs /50 truncate">{description || url}</p>
      </div>
    </a>
  );
};
...
export const LinkBlock: React.FC<LinkBlockProps> = ({ widget, onUpdate }) => {
  ...
  const handleSave = () => {
    ...
    if (!newTitle) {
      if (newProvider === "file")
        newTitle = urlInput.split("/").pop() || "File";
      if (newProvider === "github")
        newTitle = urlInput.replace("https://github.com/", "");
    }
  ...
  const renderContent = () => {
    ...
    switch (provider) {
      case "youtube":
        return <YoutubeRenderer url={url} title={title} />;
      case "vimeo":
        return <VimeoRenderer url={url} title={title} />;
      case "loom":
        return <LoomRenderer url={url} title={title} />;
      case "twitter":
        return <TwitterRenderer url={url} title={title} />;
      case "instagram":
        return <InstagramRenderer url={url} title={title} />;
      case "reddit":
        return <RedditRenderer url={url} title={title} />;
      case "github":
        return <GithubRenderer url={url} title={title} />;
      case "spotify":
        return <SpotifyRenderer url={url} />;
      case "figma":
        return <FigmaRenderer url={url} title={title} />;
      case "codepen":
        return <CodepenRenderer url={url} title={title} />;
      case "map":
        return <MapRenderer url={url} title={title} />;
      case "image":
        return <ImageRenderer url={url} title={title} />;
      case "video":
        return <VideoRenderer url={url} title={title} />;
      case "file":
        return <FileRenderer url={url} title={title} />;
      default:
        return (
          <GenericRenderer url={url} title={title} description={description} />
        );
    }
  };
    ...
    <div className="w-full relative group  text-black border-black font-title border rounded-lg overflow-hidden  transition-colors">
      {isEditing ? (
        <div className="p-4 flex flex-col text-black gap-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              size="sm"
              variant="primary"
              className=" justify-start bg-[#010101]  hover:bg-[#010101]/80"
              onClick={handleSave}
            >
        ...
        <div className="relative">
          <div className="absolute top-2 right-2  text-black hover:bg-black/20 transition-opacity z-10 flex gap-1">
            <Button
              variant="ghost"
              size="sm"
