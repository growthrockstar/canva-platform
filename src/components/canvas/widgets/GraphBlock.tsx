...
export const GraphBlock: React.FC<GraphBlockProps> = ({ widget, onUpdate }) => {
    ...
    const [isConfiguring, setIsConfiguring] = useState(!widget.graphConfig?.tableId);
    ...
    const chartData = useMemo(() => {
        ...
        // Get COMPUTED data 
        const computedData = sheetEngine.getComputedData(selectedTable.id, selectedTable.data.length, selectedTable.data[0].length);
    ...
    if (isConfiguring || !widget.graphConfig) {
        return (
            <div className="p-4 bg-gray-900/50 rounded-lg border border-white/10">
                <h3 className="text-lg font-semibold mb-4 text-white">Configurar Gráfico</h3>
                <ConfigForm
        ...
        <div className="relative group p-4 border border-white/5 rounded-lg bg-white/5">
            ...
            <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                    {renderChart(widget.graphConfig.chartType, chartData, widget.graphConfig.dataColumns, headers)}
                </ResponsiveContainer>
            </div>
        </div>
    );
...
const renderChart = (type: string, data: any[], dataColumns: number[], headers: string[]) => {
...
const ConfigForm = ({ availableTables, initialConfig, onSave, onCancel }: any) => {
    const [tableId, setTableId] = useState(initialConfig?.tableId || (availableTables.length > 0 ? availableTables[0].id : ''));
    const [chartType, setChartType] = useState<'bar' | 'line' | 'pie' | 'area'>(initialConfig?.chartType || 'bar');

    // Derive columns from selected table
    const selectedTable = availableTables.find((t: any) => t.id === tableId);
    const headers = selectedTable?.data[0] || [];

    const [xAxisColumn, setXAxisColumn] = useState<number>(initialConfig?.xAxisColumn || 0);
    const [dataColumns, setDataColumns] = useState<number[]>(initialConfig?.dataColumns || [1]);
    ...
    const toggleDataColumn = (index: number) => {
        if (dataColumns.includes(index)) {
            if (dataColumns.length > 1) setDataColumns(dataColumns.filter(i => i !== index));
        } else {
    ...
    return (
        <div className="space-y-4">
            <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Seleccionar Tabla</label>
                <select
                    value={tableId}
                ...
                >
                    {availableTables.map((t: any) => (
                        <option key={t.id} value={t.id}>{t.title} (ID: {t.id.slice(0, 4)})</option>
                    ))}
                </select>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1">Eje X (Etiquetas)</label>
                    <select
                        value={xAxisColumn}
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-1">Datos (Series)</label>
